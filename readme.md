...
- **time_evolution.py**: 控制量子系统的时间演化
  ...
  - `run_epr_generation_process(state_manager, params)`：执行基于Barren-Kok协议的EPR生成过程
...
#### 量子协议 (protocols/)
- **two_cavity_epr.py**: 实现基于Barren-Kok协议的双腔系统EPR纠缠态生成
...
#### T1阶段：EPR制备
通过Barren-Kok协议，在B和C之间建立EPR对：
1. 对腔1内的原子B和腔2内的原子C分别施加激光脉冲，各自有小概率产生单光子
2. 两个腔发出的光子在分束器处干涉，探测器D1或D2探测到单光子
3. 探测事件由`protocols/two_cavity_epr.py`中的处理函数处理
4. 当且仅当探测到一个光子且无法区分光子来源时，系统被投影到B和C的纠缠态，如(|g⟩_B|e⟩_C ± |e⟩_B|g⟩_C)/√2
...
2. **仿真进程**:
   ...
   - 通过**`protocols/two_cavity_epr.py`**执行基于Barren-Kok协议的EPR生成
